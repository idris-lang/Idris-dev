...
multInverseInversesL l r =
    ...
    let step2 = replace {P = \x => x = neutral} (ringOpIsDistributiveR l (inverse l) r) step1 in
    let step3 = cong {f = ((inverse (l <.> r)) <+>)} step2 in
    let step4 = replace {P = \x => inverse (l <.> r) <+> (l <.> r <+> inverse l <.> r) = x} (monoidNeutralIsNeutralL (inverse (l <.> r))) step3 in
    let step5 = replace {P = \x => x = inverse (l <.> r)} (semigroupOpIsAssociative (inverse (l <.> r)) (l <.> r) (inverse l <.> r)) step4 in
...
multInverseInversesR l r =
    ...
    let step2 = replace {P = \x => x = neutral} (ringOpIsDistributiveL l r (inverse r)) step1 in
    let step3 = cong {f = ((inverse (l <.> r)) <+>)} step2 in
    let step4 = replace {P = \x => inverse (l <.> r) <+> (l <.> r <+> l <.> inverse r) = x} (monoidNeutralIsNeutralL (inverse (l <.> r))) step3 in
