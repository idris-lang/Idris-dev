...
parsePositive s = parsePosTrimmed (trim s) where
  parsePosTrimmed s with (strM s)
    ...
    parsePosTrimmed (strCons x xs) | (StrCons x xs) = 
      if (x == '+') 
        then map fromInteger (parsePosAux (unpack xs) (cast {from=Int} 0))
          ...
          where
            ...
            parsePosAux (c :: cs) acc = 
              if (c >= '0' && c <= '9') 
                then parsePosAux cs ((acc * (cast {from=Int} 10)) + (cast ((ord c) - (ord '0'))))
                else Nothing
...
parseInteger s = parseIntTrimmed (trim s) where
  parseIntTrimmed s with (strM s)
    ...
    parseIntTrimmed (strCons x xs) | (StrCons x xs) = 
      if (x == '-') 
        then map (\y => negate (fromInteger y)) (parseIntegerAux (unpack xs) (cast {from=Int} 0))
        else if (x == '+') 
          then map fromInteger (parseIntegerAux (unpack xs) (cast {from=Int} 0))
            ...
            where
              ...
              parseIntegerAux (c :: cs) acc = 
                if (c >= '0' && c <= '9') 
                  then parseIntegerAux cs ((acc * (cast {from=Int} 10)) + (cast ((ord c) - (ord '0'))))
                  else Nothing
