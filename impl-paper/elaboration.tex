...
$\MO{NewTerm}\:\vt$ updates the proof state to:
...
$\Gamma$, returing its normal form.
\item 
$\Unify\:\ve_1\:\ve_2$, 
...
well-typed. Tactics may also have the side effect of updating the proof state,
...
\MO{Solve}_\Gamma & (\guess{\vx}{\vT}{\vv}\SC\ve) &
   \MoRet{\RW{return}\:\ve[\vv/\vx]\hg\RW{if}\:\MO{Pure}\:\vv}
}
}
...
\MO{Try}_\Gamma & \VV{t1} & \VV{t2} & \vt &
   \MoRet{\AR{\VV{t1}_\Gamma\:\vt\hg\RW{if}\:\VV{t1}\:\mbox{succeeds} \\
              \VV{t2}_\Gamma\:\vt\hg\RW{otherwise}}}
}
}
...
binding if the goal is a function type:
...
locates the appropriate sub-term, applies the tactic with the local context
and local sub-term, and
replaces it with the term returned by the
tactic, updates the hole queue in the proof state, and updates holes which have
