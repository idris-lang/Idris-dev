...
The idea is to extend the syntax for binders with a \remph{hole}
...
the problem is to be solved, and the expressions to be unified.
\end{itemize}
...
to $\Gamma$
\end{itemize}
\item \demph{Unification}, which unifies two terms (potentially solving sub-goals) relative to $\Gamma$. This may introduce new unification problems
to the proof state
...
$\PrimUnify\:\ve_1\:\ve_2$ may give one of the following results:
...
If $\tp$ is empty, there is a solution unifying $\ve_1$ and $\ve_2$ in
the context $\Gamma$.
...
We also define primitive tactics for constructing binders. Creating a
$\lambda$-binding requires that the goal normalises to a function type:
...
\TT{} definition for the identity function:
...
\DM{
\MO{ElabClause}\;(\vx\;\ttt\;=\;\ve)\;\mq\;\ldots
}
...
$\iwhere$ clauses. In this case, after elaborating the left hand side, the 
pattern-bound variables are added as extra arguments to the declarations in the
$\iwhere$ block, which are then recursively elaborated before the right hand
side.
...
%\footnote{This is sometimes referred to
%as ``magic \texttt{with}''.} 
