...
\ref{metatheory}.  Specifically: the \demph{Church-Rosser} property
(i.e. that distinct reduction sequences lead to the same normal form); 
\demph{subject
reduction} (i.e. that computation preserves type); the \demph{cut} property
(i.e. that $\RW{let}$-bound terms may be substituted into their scope); and
\demph{strengthening} (i.e. that introducing new definitions into scope does
