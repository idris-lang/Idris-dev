...
types and function names must begin with a capital letter or not. Indeed, such
a distinction would not necessarily be helpful, since function names may
appear in pattern context if forced by some dependency. 
%Function
...
so that the names are different internally. Names can be used qualified by
a namespace (e.g. \texttt{Vect.Nil}) or unqualified. If unqualified, it must
be possible to distinguish between names by type.
