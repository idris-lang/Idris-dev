...
defunctionalise nexttag defs
     = let pairs = toAlist defs
           -- sort newcons so that EVAL and APPLY cons get sequential tags
           (allD, (enames, anames)) = runState (mapM (addApps defs) pairs) ([], [])
           anames' = sort (nub anames)
           enames' = nub enames
           newecons = sortBy conord $ concatMap (toCons enames') (getFn pairs)
           newacons = sortBy conord $ concatMap (toConsA anames') (getFn pairs)
...
addApps _    (n, LConstructor _ t a)
    = return (n, DConstructor n t a)
addApps defs (nm, LFun _ _ fargs body)
    = do e' <- aa fargs body
         return (nm, DFun nm fargs e')
  where
    ...
    aa env (LApp tc (LV (Glob n)) args)
       = do args' <- mapM (aa env) args
            case lookupCtxtExact n defs of
                Just (LConstructor _ _ _) -> return $ DApp tc n args'
    ...
    aa env (LLazyApp n args)
       = do args' <- mapM (aa env) args
            case lookupCtxtExact n defs of
                Just (LConstructor _ _ _) -> return $ DApp False n args'
                Just (LFun _ _ as _) -> let arity = length as in
    ...
    aa env (LOp f args) = do args' <- mapM (aa env) args
                             return $ DOp f args'
    aa _   LNothing = return DNothing
    aa _   (LError e) = return $ DError e
    ...
    fixApply tc n args ar = case compare (length args) ar of
        EQ -> return $ DApp tc n args
        LT -> do (ens, ans) <- get
                 let alln = map (\x -> (n, x)) [length args .. ar]
                 put (ens, alln ++ ans)
                 return $ DApp tc (mkUnderCon n (ar - length args)) args
        GT -> return $ chainAPPLY (DApp tc n (take ar args)) (drop ar args)

    fixLazyApply n args ar = case compare (length args) ar of
        EQ -> do (ens, ans) <- get
                 put (n : ens, ans)
        ...
        GT -> return $ chainAPPLY (DApp False n (take ar args)) (drop ar args)
...
--    needsEval x (DCase _ e alts) = needsEval x e || any nec alts
...
--    needsEval x (DOp _ args) = any (needsEval x) args
...
mkEval xs = (sMN 0 "EVAL", DFun (sMN 0 "EVAL") [sMN 0 "arg"]
  ...
  where
    evalCase (_, _, EvalCase x) = Just (x (sMN 0 "arg"))
    evalCase _ = Nothing
...
mkApply xs = (sMN 0 "APPLY", DFun (sMN 0 "APPLY") [sMN 0 "fn", sMN 0 "arg"]
  ...
  where
    applyCase (_, _, ApplyCase x) = Just x
    applyCase _ = Nothing
...
instance Show DExp where
   show dexp = show' [] dexp where
     show' env (DV (Loc i)) = "var " ++ env!!i
     show' _   (DV (Glob n)) = "GLOB " ++ show n
     ...
     show' env (DC loc _ n args) = atloc loc ++ "CON " ++ show n ++ "(" ++ showSep ", " (map (show' env) args) ++ ")"
       where atloc Nothing = ""
             atloc (Just l) = "@" ++ show (LV l) ++ ":"
     show' _   (DProj t i) = show t ++ "!" ++ show i
...
mkBigCase _ maxBranches arg branches
   | length branches <= maxBranches = DChkCase arg branches
...
groupsOf x xs = let (batch, rest) = span (tagLT (x + tagHead xs)) xs in
  ...
  where tagHead (DConstCase (I i) _ : _) = i
        ...
        tagLT _ (DDefaultCase _) = False

dumpDefuns :: DDefs -> String
dumpDefuns ds = showSep "\n" $ map showDef (toAlist ds)
  where showDef (_, DFun fn args body)
            = show fn ++ "(" ++ showSep ", " (map show args) ++ ") = \n\t" ++
              show body ++ "\n"
        showDef (_, DConstructor n t _) = "Constructor " ++ show n ++ " " ++ show t
