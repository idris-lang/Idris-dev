...
addTags initial ds = tag initial ds []
...
lift _   (LV v) = return (LV v) -- Lifting happens before these can exist...
...
lift _   (LLazyExp (LConst c)) = return (LConst c)
...
lift _   (LConst c) = return (LConst c)
lift env (LForeign t s args) = do args' <- mapM liftF args
                                  return (LForeign t s args')
  where
    liftF (t', e) = do e' <- lift env e
                       return (t', e')
lift env (LOp f args) = do args' <- mapM (lift env) args
                           return (LOp f args')
lift _   (LError s) = return $ LError s
lift _   LNothing = return LNothing

allocUnique :: LDefs -> (Name, LDecl) -> (Name, LDecl)
allocUnique _    p@(_, LConstructor _ _ _) = p
allocUnique defs (nm, LFun opts fn fargs body)
    = let e' = evalState (findUp body) [] in
          (nm, LFun opts fn fargs e')
  where
    ...
    doUpAlt n (LConCase i t args rhs)
           = do avail <- get
                ...
                put avail
                return $ LConCase i t args rhs'
    doUpAlt _ (LConstCase i rhs) = LConstCase i <$> findUp rhs
    doUpAlt _ (LDefaultCase rhs) = LDefaultCase <$> findUp rhs

    findVar _ [] _ = return Nothing
...
usedIn env (LCon v _ _ args) = let rest = concatMap (usedIn env) args in
                                   case v of
                                      Nothing -> rest
                                      Just (Glob n) -> usedArg env n ++ rest
usedIn env (LProj t _) = usedIn env t
usedIn env (LCase _ e alts) = usedIn env e ++ concatMap usedInA alts
  where usedInA (LConCase _ _ _ e') = usedIn env e'
        usedInA (LConstCase _ e')   = usedIn env e'
        usedInA (LDefaultCase e')   = usedIn env e'
usedIn env (LForeign _ _ args) = concatMap (usedIn env) (map snd args)
usedIn env (LOp _ args) = concatMap (usedIn env) args
usedIn _   _ = []
...
instance Show LExp where
   show e = show' [] "" e where
     show' env _   (LV (Loc i)) = env!!i
     show' _   _   (LV (Glob n)) = show n
     ...
     show' _   _   (LProj t i) = show t ++ "!" ++ show i

     show' env ind (LCon loc _ n args)
     ...
     show' _   _   (LConst c) = show c

     show' env ind (LForeign ty n args) = concat
            [ "foreign{ "
            ,       show n ++ "("
            ,           showSep ", " (map (\(argTy,x) -> show' env ind x ++ " : " ++ show argTy) args)
            ,       ") : "
            ,       show ty
            , " }"
            ]
     ...
     show' _   _   (LError s) = "error " ++ show s
     show' _   _   LNothing = "____"
