...
isTType :: Context -> Env -> Term -> TC ()
isTType ctxt env tm = isTType' (normalise ctxt env tm)
    where isTType' (TType _) = return ()
          isTType' tm = fail (showEnv env tm ++ " is not a TType")
