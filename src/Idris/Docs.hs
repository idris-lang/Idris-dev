...
pprintFD ist (FD n doc args ty f)
    = nest 4 (prettyNamePossParen True (ppopt_impl ppo) [] n <+> colon <+>
...
pprintDocs ist (ClassDoc n doc meths params instances superclasses)
           = nest 4 (text "Type class" <+> prettyNamePossParen True (ppopt_impl ppo) [] n <>
  ...
  where
    ...
    prettyParameters = if any (isJust . snd) params
                       then vsep (map (\(nm,md) -> prettyNamePossParen True False params' nm <+> maybe empty showDoc md) params)
                       else hsep (punctuate comma (map (prettyNamePossParen True False params' . fst) params))
