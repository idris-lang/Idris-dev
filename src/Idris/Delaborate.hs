...
pshow i (CantUnify _ x y e sc s) 
    = let imps = opt_showimp (idris_options i) in
        "Can't unify " ++ showImp imps False (delab i x)
          ++ " with " ++ showImp imps False (delab i y) ++
--         " (" ++ show x ++ " and " ++ show y ++ ") " ++
        case e of
            Msg "" -> ""
            _ -> "\n\nSpecifically:\n\t" ++ pshow i e ++ 
...
pshow i (CantConvert x y env) 
    = let imps = opt_showimp (idris_options i) in
          "Can't convert " ++ showImp imps False (delab i x) ++ " with " 
                 ++ showImp imps False (delab i y) ++
                 if (opt_errContext (idris_options i)) then showSc i env else ""
pshow i (UnifyScope n out tm env) 
    = let imps = opt_showimp (idris_options i) in
          "Can't unify " ++ show n ++ " with " 
                 ++ showImp imps False (delab i tm) ++ " as " ++ show out ++
                 " is not in scope" ++
                 if (opt_errContext (idris_options i)) then showSc i env else ""
...
pshow i (NonFunctionType f ty)
    = let imps = opt_showimp (idris_options i) in
          showImp imps False (delab i f) ++ " does not have a function type ("
            ++ showImp imps False (delab i ty) ++ ")"
pshow i (CantIntroduce ty)
    = let imps = opt_showimp (idris_options i) in
          "Can't use lambda here: type is " ++ showImp imps False (delab i ty)
...
pshow i (IncompleteTerm t) = "Incomplete term " ++ showImp True False (delab i t)
