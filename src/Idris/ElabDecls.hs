...
elabType info syn doc fc opts n ty' = {- let ty' = piBind (params info) ty_in 
      ...
      do checkUndefined fc n
         ctxt <- getContext
         i <- getIState
         logLvl 3 $ show n ++ " pre-type " ++ showImp True False ty'
         ty' <- addUsingConstraints syn fc ty'
         ty' <- implicit syn n ty'
         let ty = addImpl i ty'
         logLvl 2 $ show n ++ " type " ++ showImp True False ty
...
elabCon info syn tn codata (doc, n, t_in, fc)
    = do checkUndefined fc n
         ...
         let t = addImpl i t_in
         logLvl 2 $ show fc ++ ":Constructor " ++ show n ++ " : " ++ showImp True False t
         ((t', defer, is), log) <- tclift $ elaborate ctxt n (TType (UVal 0)) []
                                            (erun fc (build i info False n t))
                   ...
                   if cov  
                      then do missing <- genClauses fc n (map getLHS pdef) cs
                              ...
                              logLvl 2 $ "Must be unreachable:\n" ++ 
                                          showSep "\n" (map (showImp True False) missing') ++
                                         "\nAgainst: " ++
                                          showSep "\n" (map (\t -> showImp True False (delab ist t)) (map getLHS pdef))
...
elabVal info aspat tm_in
   = do ctxt <- getContext
        ...
        logLvl 10 (showImp True False tm)
        -- try:
...
elabClause info tcgen (cnum, PClause fc fname lhs_in withs rhs_in whereblock) 
   = do ctxt <- getContext
        ...
        logLvl 5 ("LHS: " ++ show fc ++ " " ++ showImp True False lhs)
        ...
        logLvl 5 (showImp True False (expandParams decorate newargs defs (defs \\ decls) rhs_in))
        let rhs = addImplBoundInf i (map fst newargs) (defs \\ decls)
                                 (expandParams decorate newargs defs (defs \\ decls) rhs_in)
        logLvl 2 $ "RHS: " ++ showImp True False rhs
...
elabClause info tcgen (_, PWith fc fname lhs_in withs wval_in withblock) 
   = do ctxt <- getContext
        ...
        i <- getIState
        let lhs = addImplPat i lhs_in 
        logLvl 5 ("LHS: " ++ showImp True False lhs)
        ((lhs', dlhs, []), _) <- 
            tclift $ elaborate ctxt (MN 0 "patLHS") infP []
              (erun fc (buildTC i info True tcgen fname (infTerm lhs))) 
        let lhs_tm = orderPats (getInferTerm lhs')
        ...
        logLvl 5 ("Checking " ++ showImp True False wval)
        ...
        logLvl 5 ("New RHS " ++ showImp True False rhs)
  ...
  where
    ...
    mkAux wname toplhs ns ns' (PClause fc n tm_in (w:ws) rhs wheres)
        = do i <- getIState
             ...
             logLvl 2 ("Matching " ++ showImp True False tm ++ " against " ++ 
                                      showImp True False toplhs)
  ...
  where
    ...
    tdecl allmeths (PTy doc syn _ o n t) 
           = do t' <- implicit' syn allmeths n t
                logLvl 5 $ "Method " ++ show n ++ " : " ++ showImp True False t'
    ...
    cfun cn c syn all con
        = do let cfn = UN ('@':'@':show cn ++ "#" ++ show con)
             ...
             iLOG (showImp True False ty)
             iLOG (showImp True False lhs ++ " = " ++ showImp True False rhs)
             i <- getIState
...
checkInferred fc inf user =
     do logLvl 6 $ "Checked to\n" ++ showImp True False inf ++ "\n\nFROM\n\n" ++
                                     showImp True False user
